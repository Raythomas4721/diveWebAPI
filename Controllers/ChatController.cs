...
namespace diveWebAPI.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetChatResponse([FromBody] ChatRequest request)
        {
            ...
            var systemPrompt = $"你是一個潛水裝備與課程客服機器人，根據以下資料和對話歷史，簡潔回答客戶關於價格、運送費、尺寸、顏色、性別、庫存、課程價格、總名額、課程難易度和開始日期的問題，並根據要求推薦適合的潛水裝備或課程（浮潛、自由潛水、水肺潛水）。\n" +
                  $"### 產品資料\n{productList}\n" +
                  $"### 課程資料（每類型挑選一個代表課程）\n{courseList}\n" +
                  $"### 回答規則\n" +
                  "1. 回答必須簡短直接，避免冗長描述。\n" +
                  "2. 參考對話歷史，根據用戶前文提到的需求或偏好提供連貫回應，例如用戶問過面鏡後再問價格時，直接針對該面鏡回覆。\n" +
                  "3. 推薦課程時，優先選擇價格最低或開始時間最近的課程，並根據需求匹配類型（例如自由潛水選相關課程）。\n" +
                  "4. 推薦潛水裝備時，必須按以下格式包含所有指定資訊：'商品名稱: XXX, 價格 $YYY, 運送費 $ZZZ, 庫存: 有貨/無貨'，最多推薦 3 項，且不得省略商品名稱。\n" +
                  "5. 若用戶說「想了解更多」：\n" +
                  "   - 若前文提到「課程」「課」「潛水課程」，回覆：'請前往我們的課程詳情頁了解更多：http://localhost:4200/#/courses'\n" +
                  "   - 若前文提到「商品」「裝備」「潛水裝備」，回覆：'請前往我們的商品頁了解更多：http://localhost:4200/#/shop'\n" +
                  "   - 若無法判斷，問：'請問您想了解商品還是課程的更多資訊？'\n" +
                  "6. 若問題超出資料範圍或無法回答，回覆：'抱歉，我目前不知道'\n";
